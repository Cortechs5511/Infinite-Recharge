...
import frc.paths.TrenchComplex;
import frc.robot.subsystems.Drive;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;
import edu.wpi.first.wpilibj2.command.CommandBase;

//3, 11 for the index position in TrenchComplex
public class TrenchComplexPath extends CommandBase {
  @SuppressWarnings({ "PMD.UnusedPrivateField", "PMD.SingularField" })
  private Drive m_drive;
  private double[][] points = TrenchComplex.points;
  private double leftReference, rightReference;
  private double leftTarget, rightTarget;
  private static final double TICKS_PER_FOOT = 688.8 / Math.PI;
  private int row = 1;

  public TrenchComplexPath(Drive drive) {
    m_drive = drive;
