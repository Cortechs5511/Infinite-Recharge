...
public class TowerSimpleNoL extends SequentialCommandGroup {
	@SuppressWarnings({ "PMD.UnusedPrivateField", "PMD.SingularField" })
	public TowerSimpleNoL(double startDelay, Shooter shooter, Feeder feeder, Limelight limelight, Drive drive) {
		addCommands(
			new Brake(drive), // sets all motors to brake mode
			new WaitCommand(startDelay), // optional start delay
			new ShootAuto(-1, shooter, feeder, limelight), // fast shoot
